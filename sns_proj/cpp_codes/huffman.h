...
void huffman::coding_save()
{
	...
	{//get all characters and their huffman codes for output
		...
		string _s = s.substr(0, 8);
		in += (char)binary_to_decimal(_s);										
		for (int i = 0; i < 15; i++)
		{//cut into 8-bit binary codes that can convert into saving char needed for binary file
			s = s.substr(8);
			string __s = s.substr(0, 8);
			in += (char)binary_to_decimal(__s);
		}
		temp.pop();
	}
	s.clear();
	...
	{//get the huffman code
		...
		while (s.size() > 8)
		{//cut into 8-bit binary codes that can convert into saving char needed for binary file
			string _s_ = s.substr(0, 8);
			in += (char)binary_to_decimal(_s_);
			s = s.substr(8);
		}
		in_file.get(id);
	}
